...
연애는 스펙처럼 관리되지 않고, 결혼이나 동거는 더더욱 계획표대로 움직이지 않는다. 하지만 누군가와 삶의 비용을 나눈다는 상상은 생각보다 현실적이고, 생각보다 무섭다.`,
  ...
  },
  {
    ...
    choices: [
      { id: "refuse_pyramid", label: "정중하게 거절하고 자리에서 일어난다.", summary: "위험한 제안을 뿌리치고 평판을 지켰다.", statDelta: { reputation: 3, mental: 2, wealth: -2 }, relationshipDelta: [{ name: "준호", trust: -15 }], flagDelta: { pyramidRefused: true } },
      { id: "hear_more", label: "일단 설명을 더 들어보기로 한다.", summary: "호기심에 다단계 설명을 들었지만 빠져나오기 어려워졌다.", statDelta: { wealth: 1, reputation: -3, mental: -3 }, relationshipDelta: [{ name: "준호", trust: 3 }], flagDelta: { pyramidHeard: true } },
    ],
    tags: ["다단계", "사기", "위험"],
    source: "STATIC" as const,
  },
  ...
  },
  {
    ...
    choices: [
      { id: "refuse_underworld", label: "고맙지만 괜찮다고 말하고 빨리 집에 간다.", summary: "당신은 위험한 제안을 뿌리치고 안전한 길을 선택했다.", statDelta: { reputation: 2, mental: 2, wealth: -3 }, relationshipDelta: [{ name: "재석", trust: -10 }], flagDelta: { underworldRefused: true } },
      { id: "accept_underworld", label: "무슨 일인지 궁금해서 한 번 들어본다.", summary: "당신은 위험한 세계에 첫발을 내디뎠다.", statDelta: { wealth: 6, practical: 2, reputation: -5, mental: -4 }, relationshipDelta: [{ name: "재석", trust: 12 }], flagDelta: { underworldEntered: true } },
    ],
    tags: ["범죄", "알바", "위험"],
    source: "STATIC" as const,
  },
  ...
  },
  {
    ...
    choices: [
      ...
      { id: "open_usb", label: "USB 내용물을 확인한다.", summary: "USB 안의 정보를 확인하고 위험한 지식을 얻었다.", statDelta: { practical: 5, reputation: -3, mental: -2, wealth: 2 }, relationshipDelta: [], flagDelta: { usbInvestigation: "opened" } },
    ],
    tags: ["의문", "위험", "선택"],
    source: "STATIC" as const,
  },
  {
    ...
    choices: [
      { id: "refuse_gambling", label: "쪽지를 찢어 버리고 없는 일로 한다.", summary: "위험한 도박의 유혹을 뿌리치고 깨끗한 길을 선택했다.", statDelta: { reputation: 3, mental: 2, wealth: -1 }, relationshipDelta: [{ name: "수진", trust: -10 }], flagDelta: { gamblingRefused: true } },
      { id: "try_gambling", label: "호기심에 한 번 나가본다.", summary: "작은 도박이 큰 빚으로 이어질지도 모른다.", statDelta: { wealth: 5, practical: 1, reputation: -4, mental: -5 }, relationshipDelta: [{ name: "수진", trust: 8 }], flagDelta: { gamblingTried: true } },
    ],
    tags: ["도박", "위험", "선배"],
    source: "STATIC" as const,
  },
  ...
  },
  {
    ...
    choices: [
      { id: "start_exercise", label: "헬스장에 등록하고 규칙적으로 운동한다.", summary: "운동을 시작하며 컨디션 회복에 힘썼다.", statDelta: { health: 6, mental: 2, wealth: -3, practical: -1 }, relationshipDelta: [], flagDelta: { healthRoutine: "exercise" } },
      { id: "reduce_schedule", label: "약속과 알바를 줄이고 휴식을 늘린다.", summary: "일정을 줄이고 회복에 집중했다.", statDelta: { health: 4, mental: 3, wealth: -2, reputation: -2 }, relationshipDelta: [], flagDelta: { healthRoutine: "rest" } },
  ...
  },
  {
    ...
    choices: [
      { id: "accept_personal_training", label: "개인 운동을 받아들인다.", summary: "현우 트레이너와 가까워지며 운동과 관계 모두 발전했다.", statDelta: { health: 5, charm: 2, wealth: -2 }, relationshipDelta: [{ name: "현우", trust: 7 }], flagDelta: { personalTraining: "accepted" } },
  ...
  },
  {
    ...
    choices: [
      { id: "honest_with_mom", label: "솔직하게 불안하다고 말씀드린다.", summary: "어머니께 솔직한 심정을 털어놓고 위로를 받았다.", statDelta: { mental: 4, wealth: 1 }, relationshipDelta: [{ name: "부모님", trust: 8 }], flagDelta: { momCall: "honest" } },
...
교수님은 출석을 부르다 당신의 이름 앞에서 잠깐 멈춘다. 옆자리의 민하가 조용히 물병을 밀어주고, 휴대폰에는 점장의 주말 대타 메시지가 다시 떠 있다. 돈을 버는 일은 끝난 뒤에도 하루를 계속 점령한다는 걸, 당신은 이제 몸으로 알고 있다.`,
    choices: [
      { id: "ask_shift_boundary", label: "점장에게 주말 대타는 어렵다고 선을 긋는다.", summary: "당신은 돈보다 수업 리듬을 지키기 위해 알바 시간을 제한했다.", statDelta: { health: 4, mental: 2, wealth: -4, reputation: -1 }, relationshipDelta: [{ name: "명수", trust: -4 }], flagDelta: { partTimeBoundary: true, moneyThread: "strained" } },
      { id: "hide_fatigue", label: "피곤한 티를 숨기고 대타까지 다시 맡는다.", summary: "당신은 생활비를 붙잡기 위해 피로를 더 깊이 숨겼다.", statDelta: { wealth: 5, practical: 1, health: -6, mental: -4 }, relationshipDelta: [{ name: "민하", trust: -3 }], flagDelta: { sleepDebtThread: "deepened", moneyThread: "urgent" } },
    ],
    tags: ["알바", "건강", "학업"],
    source: "STATIC",
...
약속 장소에 도착하자 봉투 하나와 낯선 이름이 적힌 메모가 놓여 있다. 재석은 별일 아니라며 웃지만, 당신은 이 일이 단순한 심부름이 아니라 누군가의 약점을 옮기는 일일 수도 있다는 생각을 지우지 못한다. 지금 물러서면 손해를 볼 것이고, 계속하면 더 깊은 문이 열린다.`,
    choices: [
      { id: "walk_away_underworld", label: "봉투를 내려놓고 더는 못 하겠다고 말한다.", summary: "당신은 위험한 돈에서 빠져나오려 했지만 재석의 눈밖에 났다.", statDelta: { reputation: 3, mental: -3, wealth: -5, health: -1 }, relationshipDelta: [{ name: "재석", trust: -24 }], flagDelta: { underworldExitAttempt: true, crimeThread: "exit_attempt" } },
      { id: "deliver_envelope", label: "아무것도 묻지 않고 봉투를 전달한다.", summary: "당신은 위험한 심부름을 받아들이며 회색지대에 더 깊이 들어갔다.", statDelta: { wealth: 9, practical: 2, reputation: -10, mental: -6 }, relationshipDelta: [{ name: "재석", trust: 14 }], flagDelta: { underworldDebt: true, crimeThread: "entangled" } },
    ],
    tags: ["범죄", "알바", "위험"],
    source: "STATIC",
...
휴대폰에는 카드값 알림이 떠 있고, 머릿속에서는 잃은 돈을 한 번에 되찾는 장면이 자꾸 재생된다. 그러나 옆 계단에는 같은 과 후배가 앉아 있어, 당신이 누구와 이야기하는지 볼 수도 있다. 한 번 더 들어가는 선택은 돈만이 아니라 평판과 관계까지 건다.`,
    choices: [
      { id: "block_gambling_contact", label: "수진의 연락처를 차단하고 후배에게도 조심하라고 말한다.", summary: "당신은 도박 연결을 끊고 주변에 경고했다.", statDelta: { reputation: 4, mental: 2, wealth: -3, charm: -1 }, relationshipDelta: [{ name: "수진", trust: -25 }], flagDelta: { gamblingCutOff: true, crimeThread: "resisted" } },
      { id: "chase_losses", label: "이번 한 번만 손실을 메우겠다고 따라간다.", summary: "당신은 잃은 돈을 되찾으려다 더 큰 빚의 문을 열었다.", statDelta: { wealth: -8, mental: -8, reputation: -7, practical: 2 }, relationshipDelta: [{ name: "수진", trust: 12 }], flagDelta: { gamblingDebt: true, crimeThread: "debt" } },
    ],
    tags: ["도박", "빚", "평판"],
    source: "STATIC",
    arcIds: ["pressure", "consequence"],
...
책상 위에는 과제 자료와 아르바이트 공고, 그리고 가족에게 차마 보내지 못한 메시지가 나란히 놓여 있다. 도움을 청하면 숨통은 트일 수 있다. 대신 독립했다는 말을 스스로 조금 거둬야 한다.`,
    choices: [
      { id: "ask_family_rent_help", label: "가족에게 이번 달만 도움을 부탁한다.", summary: "당신은 자존심을 접고 가족에게 월세 도움을 요청했다.", statDelta: { wealth: 5, mental: -2, reputation: -1 }, relationshipDelta: [{ name: "부모님", trust: 5 }], flagDelta: { rentHelpAsked: true, familyPressure: "financial_help" } },
      { id: "find_more_work", label: "도움을 청하지 않고 단기 알바를 더 찾는다.", summary: "당신은 독립을 지키기 위해 노동 시간을 늘렸다.", statDelta: { wealth: 4, health: -5, mental: -3, practical: 1 }, relationshipDelta: [], flagDelta: { rentStress: true, moneyThread: "urgent" } },
    ],
    tags: ["자취", "돈", "가족"],
    source: "STATIC",
...
당신은 숟가락을 내려놓고 어느 정도까지 말할지 계산한다. 솔직해지면 도움을 받을 수도 있지만, 앞으로의 모든 선택에 의견이 따라붙을 것이다. 숨기면 오늘 밤은 조용히 지나가겠지만 압박은 더 오래 남는다.`,
    choices: [
      { id: "share_home_plan", label: "이번 학기 계획과 불안을 같이 말한다.", summary: "당신은 본가 식탁에서 진로 불안을 공유하고 현실적인 지원을 얻었다.", statDelta: { mental: 3, wealth: 2, reputation: -1 }, relationshipDelta: [{ name: "부모님", trust: 7 }], flagDelta: { familyPlanShared: true } },
...
비행기표를 결제하면 이번 학기의 많은 약속은 끊긴다. 반대로 접어두면 다시 취업 준비의 좁은 복도로 돌아가야 한다. 어느 쪽도 완전한 정답처럼 보이지 않는다.`,
    choices: [
      { id: "book_working_holiday", label: "비행기표를 결제하고 휴학 상담을 잡는다.", summary: "당신은 해외에서 다른 삶을 시험하기 위해 실제 출국 준비를 시작했다.", statDelta: { charm: 4, practical: 3, wealth: -8, reputation: -2 }, relationshipDelta: [{ name: "부모님", trust: -5 }], flagDelta: { overseasThread: "ticket_booked" } },
...
function isEventAllowedForLifeStage(event: StaticEvent, context: EventSelectionContext) {
