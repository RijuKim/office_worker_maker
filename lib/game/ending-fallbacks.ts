export function pickFallbackCareerPath(stats: Record<string, number>) {
...
export function buildFallbackLongEnding(input: {
  name: string;
  major: string;
  careerPath: string;
  ...
  const weakness = input.stats.health < 2 ? "몸을 너무 늦게 돌본 대가" :
    input.stats.mental < 2 ? "마음을 오래 방치한 대가" :
      input.stats.reputation < 2 ? "사람들 사이에 남은 오해" :
        "끝내 놓지 못한 미련";
